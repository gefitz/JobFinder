...
export class AuthenticationService {
  ...
  parametroEnvio: ParametroEnvio = {endpoint: "Login/authentication", data: {}};
  // Método para fazer login e armazenar o token
  login(tokenJson:any): void {
    this.parametroEnvio.endpoint += "?username="+ tokenJson.Email + "&password=" + tokenJson.Senha
    this.api.post(this.parametroEnvio).subscribe(
      (response) => {
        this.parametroEnvio.endpoint = "Usuario/GetAllCandidato"
        this.parametroEnvio.data = {"Nome": "Geovane"}
        this.api.post(this.parametroEnvio).subscribe(
          (response)=>{
